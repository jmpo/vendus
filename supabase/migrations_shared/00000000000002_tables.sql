...
CREATE TABLE public.chat_flows (
  ...
  product_id uuid,
  organization_id uuid NOT NULL,
  name text DEFAULT 'Flujo de Calificación'::text NOT NULL,
  description text,
...
CREATE TABLE public.webchat_agent_configs (
  ...
  system_prompt text DEFAULT 'Usted é um assistente virtual prestativo e amigável. Respondé de forma clara e objetiva.'::text,
  knowledge_base text,
  faq jsonb DEFAULT '[]'::jsonb,
  handoff_triggers text[] DEFAULT ARRAY['falar com agente'::text, 'falar com humano'::text, 'agente'::text, 'quiero comprar'::text, 'preço'::text, 'valor'::text],
  auto_handoff_enabled boolean DEFAULT true,
  greeting_message text DEFAULT 'Hola! Sou o assistente virtual. Como posso ajudar usted hoy?'::text,
...
CREATE TABLE public.webchat_widgets (
  ...
  welcome_message text DEFAULT 'Hola! Como posso ajudar?'::text,
  placeholder_text text DEFAULT 'Escribí su mensaje...'::text,
  "position" text DEFAULT 'bottom-right'::text,
  avatar_url text,
  auto_open_delay integer,
  business_hours jsonb DEFAULT '{"enabled": false}'::jsonb,
  offline_message text DEFAULT 'Estamos offline no momento. Deixe su mensaje!'::text,
