// instagram-draft
// Cria uma conexión Instagram Direct (Meta) em modo "borrador" para que o wizard
...
Deno.serve(async (req: Request) => {
  ...
  if (connection_id) {
    ...
    if (existing) {
      return json({
        ...
        webhook_url: `${supabaseUrl}/functions/v1/instagram-webhook/${existing.id}`,
  ...
  return json({
    ...
    webhook_url: `${supabaseUrl}/functions/v1/instagram-webhook/${row.id}`,
