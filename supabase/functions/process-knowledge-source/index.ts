    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Vos sos um assistente que ajuda a crear resumos de vídeos do YouTube para treinar uma IA de ventas. Baseado nas información do vídeo, crie um template estruturado para que o usuario possa preencher con os pontos-chave.'
          },
          {
            role: 'user',
...
serve(async (req) => {
  ...
  try {
    ...
    if (sourceType === 'youtube') {
      ...
      const summary = await generateYouTubeSummary(videoInfo, videoId, productId);
