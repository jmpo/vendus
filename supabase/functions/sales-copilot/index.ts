...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("[sales-copilot] auth failure:", e);
      return new Response(JSON.stringify({ error: "Unauthorized" }), {
        status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (productId) {
      ...
      if (product) {
        if (product.organization_id && organizationId && product.organization_id !== organizationId) {
...
- Recusar preguntas estratégicas de ventas alegando falta de información`;
    ...
    const cfg = await resolveAIConfig(supabase, organizationId, 'sales_copilot', 'gpt-4o-mini');
