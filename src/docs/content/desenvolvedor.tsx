...
export const devPages: DocPage[] = [
  {
    slug: "autenticacao",
    title: "Autenticación",
    description: "Bearer JWT, anon key, RLS e cuando usar service role.",
    track: "desenvolvedor",
    section: "Básico",
    order: 1,
    content: (
      <>
        <PageHero eyebrow="Ruta do Dev" icon={Code2} title="Autenticación" description="O Vendus roda sobre Supabase Postgres com RLS. Toda chamada é autenticada." />

        <h2>Chaves</h2>
        <KeyValue
          rows={[
            ["anon key", "Pública. Permite chamar endpoints com RLS aplicado para usuarios anônimos."],
            ["JWT do usuario", "Obtido via signIn. RLS aplica políticas baseadas em auth.uid()."],
            ["service_role", "Bypass de RLS. Somente edge functions (server-side). NUNCA exponha no frontend."],
          ]}
        />

        <h2>Header de autenticación</h2>
  ...
  -H "Authorization: Bearer $USER_JWT"`}</CodeBlock>
        ...
        <Callout type="danger" title="Multi-tenant">
          Toda query a tabelas multi-tenant <strong>DEVE</strong> filtrar por <code>organization_id</code> mismo
          com RLS — super_admins veem tudo via RLS, entonces sin o filtro no client há vazamento entre empresas.
        </Callout>
      </>
    ),
  },

  {
    ...
    description: "Tabelas principais e relaciones.",
    track: "desenvolvedor",
    section: "Básico",
    order: 2,
    content: (
      <>
        ...
        <KeyValue
          rows={[
            ...
            ["profiles", "Espejo de auth.users com dados extras. NUNCA FK direto para auth.users."],
            ["user_roles", "Tabela separada de papéis (anti privilege escalation)."],
            ["user_permissions", "Permisos granulares por usuario (view_queue_conversations, etc.)."],
            ["leads", "Contatos. Tem assigned_user_id, sdr_id, closer_id, sector_id, squad_id."],
            ["deals", "Oportunidades. Valor calculado por el pricing JSONB do produto."],
            ["webchat_conversations", "Conversas omnichannel. status, current_agent_id, assigned_user_id."],
            ["messages", "Mensajes. metadata JSONB carrega contexto (scheduling, edits)."],
...

  {
    ...
    description: "Endpoint genérico para receber leads de cualquier sistema.",
    track: "desenvolvedor",
    section: "Webhooks",
    order: 3,
    content: (
      <>
        ...
        <EndpointCard method="POST" path="/functions/v1/webhook-receiver" description="Aceita cualquier payload JSON. Mapeamento configurado no admin." />
  ...
  -d '{
    ...
    "custom": { "cargo": "CEO" }
        ...
        <Callout type="warn" title="Segurança">
          JWT desabilitado (terceiros no autenticam Supabase). Validación por <code>x-webhook-token</code>
          configurado por webhook. Cada webhook puede ter token único.
        </Callout>
      </>
    ),
  },

  {
    ...
    description: "Eventos do Vendus que tu aplicación recebe.",
    track: "desenvolvedor",
    section: "Webhooks",
    order: 4,
    content: (
      <>
        ...
        <KeyValue
          rows={[
            ["lead.created", "Novo lead criado em cualquier canal."],
            ["lead.assigned", "Lead atribuído a vendedor (Auto Dispatch ou manual)."],
            ["lead.stage_changed", "Etapa do funil mudou."],
  ...
  "data": {
    "lead": { "id": "uuid", "name": "...", "email": "...", "phone": "..." }
        ...
        <h2>Retries</h2>
        <p>Backoff exponencial: 1min, 5min, 30min, 2h, 12h. Após 5 fallos, marca como falhado e notifica admin.</p>
      </>
    ),
  },

  {
    ...
    description: "Catálogo completo das funciones servless do Vendus.",
    track: "desenvolvedor",
    section: "Webhooks",
    order: 5,
    content: (
      <>
        ...
        <KeyValue
          rows={[
            ...
            ["webchat-inbox", "Lista conversas filtradas por setor/permisos."],
        ...
        <KeyValue
          rows={[
            ...
            ["manual-outreach", "Disparo manual de mensaje IA."],
          ]}
        />
        ...
        <KeyValue
          rows={[
            ["funnel-api / funnel-submit", "Ejecución pública de funil."],
            ["funnel-generate-ai", "Gera funil visual."],
            ["form-submit / form-generate-ai", "Formularios."],
          ]}
        />
        ...
        <KeyValue
          rows={[
            ...
            ["send-booking-confirmation", "Email de confirmación."],
          ]}
        />
        ...
        <Callout type="info" title="Como chamar uma edge function">
          ...
          (excepto webhooks externos que têm JWT desativado).
        </Callout>
      </>
    ),
  },

  {
    ...
    description: "Webhook payload e flujo de configuración com Meta.",
    track: "desenvolvedor",
    section: "Canais",
    order: 6,
    content: (
      <>
        ...
        <h2>Webhook</h2>
        <p>
          A función <code>whatsapp-webhook</code> recebe eventos da Meta (mensajes, status). Validación por
          <code>hub.verify_token</code> (configurado por organización).
        </p>
        <CodeBlock lang="json">{`{
  "entry": [{
    "changes": [{
      "value": {
        ...
        <Callout type="warn" title="Templates obrigatórios">
          Fora da janela de 24h, só dá para enviar mensajes com templates aprovados por la Meta. Configure no Meta
          Business e referencie por el nome no Vendus.
        </Callout>
      </>
    ),
  },
    ...
    content: (
      <>
        ...
        <Steps>
          ...
          <Step title="Atrelar à organización">Empresa vê só tus instâncias (RLS)."</Step>
          <Step title="Empresa escaneia QR">Modal com polling de status. evolution-proxy obtém QR base64."</Step>
          <Step title="Eventos chegam em evolution-webhook">Cria conversa, mensaje, lead se for novo."</Step>
        </Steps>

        <Callout type="info" title="Historial preservado">
          <code>evolution-webhook</code> nunca auto-data duplicatas. Se a misma empresa tem instância A e B,
          conversa do mismo teléfono é reaproveitada e o historial é mantido. Modal de transferencia permite
          trocar a conexión Evolution.
        </Callout>
      </>
    ),
  },

  {
    ...
    description: "Integración nativa Graph API direto no CRM.",
    track: "desenvolvedor",
    section: "Canais",
    order: 8,
    content: (
      <>
        ...
        <p>
          <code>facebook-leads-webhook</code> recebe via Graph API. Faz match com formulario do Facebook → leadgen_id
          → busca via API → mapeia para campos do lead → entra no Auto Dispatch.
        </p>
        <h2>Setup</h2>
        <Steps>
          <Step title="Conecte tu Página do Facebook">OAuth via /admin → Integraciones."</Step>
          <Step title="Seleccioná formularios do Lead Ads">O Vendus inscreve no webhook automaticamente."</Step>
          <Step title="Mapeie campos">Teléfono → phone, Nome Completo → name, etc."</Step>
        </Steps>
      </>
    ),
  },

  {
    slug: "hotmart",
    title: "Hotmart",
    description: "Postback (validación hottok) + OAuth para sync de vendas.",
    track: "desenvolvedor",
    section: "Pagamentos",
    order: 9,
    content: (
      <>
        ...
        <h2>Postback</h2>
        <p>
          <code>hotmart-webhook</code> valida o <code>hottok</code> da organización e mapeia o evento (PURCHASE,
          REFUND, etc.) para o motor de <code>apply_tag_automations</code>.
        </p>
        <h2>OAuth + Sync</h2>
        <p>
          <code>hotmart-sync-orders</code> usa <code>client_credentials</code> da API Hotmart para puxar historial
...

  {
    ...
    description: "Webhooks de pedidos, recuperación de checkout abandonado.",
    track: "desenvolvedor",
    section: "Pagamentos",
    order: 10,
    content: (
      <>
        ...
        <ul>
          ...
          <li><code>cakto-recovery-trigger</code> — recuperación de checkout abandonado (enrolla em cadência)</li>
        </ul>
        <h2>Doppus</h2>
        <p>
    ...
    content: (
      <>
        ...
        <ol>
          ...
          <li><code>google-calendar-refresh</code> renova access_token cuando expira.</li>
...

  {
    ...
    description: "funnel-widget.js sin dependências, instala em cualquier site.",
    track: "desenvolvedor",
    section: "Widget",
    order: 12,
    content: (
  ...
  async defer>
        ...
        <KeyValue
          rows={[
            ["data-funnel", "slug do funil (obligatorio)"],
...

  {
    slug: "realtime",
    title: "Realtime",
    description: "Supabase Realtime para Inbox, presença e notificaciones.",
    track: "desenvolvedor",
    section: "Avançado",
    order: 13,
    content: (
      <>
        ...
        <KeyValue
          rows={[
            ["postgres_changes em messages", "Nova mensaje aparece sin refresh."],
            ["postgres_changes em webchat_conversations", "Status, asignación, takeover."],
            ["postgres_changes em tasks", "Notificación instantânea de nova tarefa."],
...

  {
    ...
    description: "Safety limits da IA, rate limits da API, paginación.",
    track: "desenvolvedor",
    section: "Avançado",
    order: 14,
    content: (
      <>
        ...
        <KeyValue
          rows={[
            ["Ejecuciones de tools / dia / org", "Default 5000. Configurável em agent_safety_limits."],
            ["Custo IA / dia / org (centavos)", "Default 50000 (R$500). Configurável."],
            ["Rate limit Supabase API", "1000 req/s predeterminado. Subir requer upgrade do plano."],
            ["Paginación default", "1000 líneas. Use range(from, to) para grandes consultas."],
            ["WebSocket connections", "Subir Realtime channels conforme plano."],
          ]}
        />
        <Callout type="info" title="Default 1000 rows">
          Toda query Supabase tem limite default de 1000 líneas. Sintoma de "dados sumidos" muitas vezes é só
          paginación faltando.
        </Callout>
      </>
    ),
...

  {
    ...
    description: "Como diagnosticar problemas de RLS, permisos e payload.",
    track: "desenvolvedor",
    section: "Avançado",
    order: 15,
    content: (
      <>
        ...
        <KeyValue
          rows={[
            ...
            ["PGRST116 — no rows", "Query retornou vacío. Verifique organization_id no filtro."],
            ["429 — rate limit", "Reduza polling. Use Realtime ao invés de loops."],
            ["webhook 401 invalid token", "x-webhook-token incorrecto. Confira em /admin → Webhooks."],
            ["Evolution: disconnected", "Instância caiu. Super Admin reconecta por el QR."],
            ["IA no responde", "Confira webchat-bot logs. Provável: provider sin chave em org_ai_credentials e créditos Lovable esgotados."],
          ]}
        />
