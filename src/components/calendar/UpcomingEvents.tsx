...
export function UpcomingEvents({ onViewAll, onEventClick, maxEvents = 5 }: UpcomingEventsProps) {
  ...
  const getTimeLabel = (startTime: string) => {
    ...
    if (minutesDiff <= 0) {
      return { label: 'Ahora', color: 'text-red-500', bg: 'bg-red-500/10' };
    }
    if (minutesDiff <= 30) {
      return { label: `En ${minutesDiff} min`, color: 'text-red-500', bg: 'bg-red-500/10' };
    }
    if (minutesDiff <= 60) {
      return { label: `En ${minutesDiff} min`, color: 'text-orange-500', bg: 'bg-orange-500/10' };
    }
    if (isToday(date)) {
      return { label: `Hoy, ${format(date, 'HH:mm')}`, color: 'text-yellow-500', bg: 'bg-yellow-500/10' };
    }
    if (isTomorrow(date)) {
      return { label: `Mañana, ${format(date, 'HH:mm')}`, color: 'text-green-500', bg: 'bg-green-500/10' };
    }
    return { 
      label: format(date, "EEE, d 'a las' HH:mm", { locale: es }), 
          ...
          <>
            ...
            {events && events.length > maxEvents && (
              <div className="text-center pt-2">
                <Button variant="ghost" size="sm" onClick={onViewAll} className="text-xs">
                  +{events.length - maxEvents} otros eventos
                </Button>
              </div>
            )}
          </>
