...
import type { RealtimeOps } from '@/hooks/useOperationCenter';
      ...
      <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            <MessageCircle className="h-4 w-4 text-emerald-600" /> Conversaciones
          </div>
          <Bar label="Com IA" value={c?.withAI ?? 0} max={convMax} color="bg-emerald-500" />
          <Bar label="Em Atención" value={c?.inAttendance ?? 0} max={convMax} color="bg-orange-500" />
          <Bar label="Fila Humana" value={c?.humanQueue ?? 0} max={convMax} color="bg-blue-500" />
          <Bar label="Finalizadas Hoy" value={c?.resolvedToday ?? 0} max={convMax} color="bg-muted-foreground/60" />
        </div>

        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            <Sparkles className="h-4 w-4 text-violet-600" /> Cadencias
          </div>
          <Bar label="Ativas" value={cad?.activeEnrollments ?? 0} max={cadMax} color="bg-violet-500" />
          <Bar label="Executadas Hoy" value={cad?.executedToday ?? 0} max={cadMax} color="bg-indigo-500" />
        ...
        <div className="space-y-3">
          ...
          <Bar label="Reuniões Hoy" value={ag?.todayMeetings ?? 0} max={agMax} color="bg-blue-500" />
          <Bar label="Confirmadas" value={ag?.confirmed ?? 0} max={agMax} color="bg-emerald-500" />
          <Bar label="Pendientes" value={ag?.pending ?? 0} max={agMax} color="bg-orange-500" />
        </div>
      </CardContent>
    </Card>
  );
