...
import {
  MessageCircle, MessageSquare, FormInput, Brain, GitBranch, Zap, Link,
...
const CHATBOT_BLOCKS: FunnelBlockType[] = [
  ...
  'condition', 'ab_test',
  'score', 'tag', 'create_lead', 'update_lead', 'create_task', 'schedule', 'handoff', 'end',
  'webhook', 'crm_sync',
];
...
const BLOCK_ICONS: Record<FunnelBlockType, React.ComponentType<{ className?: string }>> = {
  ...
  ai_takeover: Bot, ai_decide: Brain, ai_qualify: MessageCircle, ai_summarize: FileText,
...
export const ChatBotBlockPalette = memo(function ChatBotBlockPalette({ onAddBlock }: Props) {
  const allowed = FUNNEL_BLOCK_PALETTE.filter(b => CHATBOT_BLOCKS.includes(b.type));
