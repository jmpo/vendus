...
import { Save, RotateCcw, Loader2, MessageCircle, Smartphone, Monitor } from 'lucide-react';
...
export function ChatBotAppearanceTab({ funnel }: Props) {
  ...
  const isDirty = JSON.stringify(appearance.chat) !== JSON.stringify(initial.chat);
  ...
  const handleApplyPreset = (preset: AppearancePreset) => {
    setAppearance({
      ...appearance,
      chat: applyPresetToChannel(appearance.chat, preset, 'chat'),
    });
    ...
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="flex-1 pt-4 min-h-0 flex flex-col lg:flex-row lg:gap-4">
        <div className="order-2 lg:order-1 flex-1 min-w-0 min-h-0 lg:h-full lg:overflow-y-auto lg:pr-2 space-y-4">
          <PresetGallery channel="chat" onApply={(preset) => handleApplyPreset(preset)} />
          <AppearanceForm
            channel="chat"
            appearance={appearance.chat}
            onChange={(next) => setAppearance({ ...appearance, chat: next })}
          />
        </div>
        ...
        <div className="order-1 lg:order-2 lg:w-[440px] lg:shrink-0 lg:h-full">
          <div className="bg-muted/30 rounded-lg p-3 sm:p-4 flex flex-col gap-3 lg:h-full">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
              <AppearanceLivePreview channel="chat" appearance={appearance.chat} device={device} />
            </div>

            {appearance.chat.custom_css && (
              <style dangerouslySetInnerHTML={{ __html: appearance.chat.custom_css || '' }} />
            )}
          </div>
        </div>
      </div>
