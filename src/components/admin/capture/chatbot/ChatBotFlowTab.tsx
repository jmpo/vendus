...
export function ChatBotFlowTab({ funnel }: Props) {
    ...
    setBlocks(prev => prev.map(block => {
      ...
      switch (connectionType) {
        ...
        case 'option':
          ...
          if (block.type === 'ai_decide' && block.fecha.ai_outputs && optionIndex !== undefined) {
            const outs = [...block.fecha.ai_outputs];
            outs[optionIndex] = { ...outs[optionIndex], next_block_id: null };
            return { ...block, fecha: { ...block.fecha, ai_outputs: outs } };
          }
          return block;
    ...
    <div className="h-[calc(100vh-200px)] flex flex-col gap-2">
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        <Card className="w-56 flex-shrink-0 overflow-hidden">
          <ChatBotBlockPalette onAddBlock={handleAddBlock} />
        </Card>
