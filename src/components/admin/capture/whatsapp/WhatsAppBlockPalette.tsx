...
import {
  MessageSquare, FormInput, Brain, GitBranch, Zap, Link,
...
// Whitelist do WhatsApp — conversação completa con IA, reserva e CRM.
// Removemos: image (limitado no WhatsApp por los providers), ab_test, link (no funciona inline).
const WHATSAPP_BLOCKS: FunnelBlockType[] = [
  'message', 'buttons', 'video', 'delay',
  'input', 'quick_form',
  'ai_takeover', 'ai_decide', 'ai_qualify', 'ai_summarize', 'agent_switch',
  'condition', 'score',
  'tag', 'create_lead', 'update_lead', 'create_task', 'schedule', 'handoff', 'end',
  'webhook', 'crm_sync',
];
...
export const WhatsAppBlockPalette = memo(function WhatsAppBlockPalette({ onAddBlock }: Props) {
  const allowed = FUNNEL_BLOCK_PALETTE.filter(b => WHATSAPP_BLOCKS.includes(b.type));
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-3 py-3 border-b bg-gradient-to-r from-emerald-500/10 to-transparent shrink-0">
      ...
      <div className="px-3 py-2 border-t bg-muted/30 shrink-0">
        <p className="text-[10px] text-muted-foreground text-center">
          Flujo dispara na primera mensaje del lead
        </p>
      </div>
    </div>
  );
